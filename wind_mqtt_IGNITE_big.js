...
const port2 = new SerialPort('/dev/rfcomm0', { baudRate: 9600 });
...
port2.on("open", () => {
    port2.flush();
    console.log('Bluetooth port open');
    parser2.on('data', data =>{
        splitData = data.split(" ");
        tag = splitData[0];
        console.log('got word from potentiometer:', data);
        windSpeed = parseFloat(splitData[1]); 
        windSpeedString = windSpeed + "e"; 
        
        console.log(JSON.stringify(windSpeedString));
        port.write(windSpeedString, function(err) {
            if (err) {
...
function sendDataToAPI() {
 ...
 mqttClient.publish('ignite-windturbine-big', JSON.stringify({ rpm: dataTurbine.rpm, torque: dataTurbine.torque, wind: dataTurbine.wind, time: now }));
