...
var server = ws.createServer(function (conn) {
  ...
  conn.on('error(errObj)', function (code, errObj) {
    console.log("Connection closed")
    server.listen(8001); 
})
}).listen(8001)
...
function sendDataToAPI() {
  ...
  server.connections.forEach(function (conn) {
    conn.sendText(JSON.stringify({ rpm: dataTurbine.rpm, wind: dataTurbine.wind, torque: dataTurbine.torque, now: dataTurbine.now }));
    sendDuration = Date.now() - now;
  })
  // neeed to handle exceptions: 
 
  
  console.log(JSON.stringify({ rpm: dataTurbine.rpm, wind: dataTurbine.wind, torque: dataTurbine.torque, now: dataTurbine.now }));
  setTimeout(sendDataToAPI, 100);
